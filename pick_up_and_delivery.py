...
P = [st.pick_up_multi_delivery_dynamics(Rows, Columns, rate, drop_off_dist[p], 
                                  pick_ups[p]) for p in range(player_num)]
# P = st.pick_up_delivery_dynamics(Rows, Columns, rate, drop_off_dist, 0)
...
C = [st.pick_up_delivery_cost(Rows, Columns, A, T, pick_ups[p], drop_offs, p,
                              minimize=False) for p in range(player_num)]
...
for i in range(Iterations):
    ...
    for p in range(player_num):        
        p_cost = C[p] - st.state_congestion(Rows, Columns, mode_num, A, T, y) # 1.25 
        costs[p].append(1*p_cost)
        V, pol_new = dp.value_iteration(P[p], 1*p_cost, g=gamma, minimize=False)
        V_hist[p].append(V)
