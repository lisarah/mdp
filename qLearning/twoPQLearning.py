...
#    print (np.arange(0,S).shape)
    ...
    Q1[xCur1, aNext1] += alpha1*(gamma*maxNext1 + C1[xCur1, aNext1] - lastQ1);
    Q2[xCur2, aNext2] += alpha2*(gamma*maxNext2 + C2[xCur2, aNext2] - lastQ2);
...
VTrue1 = dp.discounted_valueIteration(P,C1,minimize = False, g = gamma);
VTrue2 = dp.discounted_valueIteration(P,C2,minimize = False, g = gamma);
VTrueMat = np.ones((S,T));
for i in range(S):
    VTrueMat[i,:] = VTrueMat[i,:]*VTrue1[i];
